...
\renewcommand\shortauthors{McNaughton, J., Crick, T. and Burd, E. }
...
% TODO Trim down - can almost totally remove this as it just repetition of the first introduction.
...
% Section~\ref{sec:orchestration} considers how the time-of-flight cameras could be used to adapt a classroom-based technology to utilise physical gestures.
% What constitutes an effective gesture is discussed in Section~\ref{subsec:gatheringGestures}.
...
% Implementation of a user generated set of gestures from observations on the focus group is detailed in Section~\ref{subsec:studyPhase1Implementation}.
% Section~\ref{sec:evaluation1} discusses a study using the implemented physical gesture control system.
% Section~\ref{subsec:studyPhase1Results} presents the results of this study and Section~\ref{subsec:studyPhase1Observations} elaborates on the implications of the results.
...
SynergyNet is a software framework designed for use in with networked touch-screen devices in a classroom environment~\cite{mcnaughton-et-al:cvm2018,mcnaughton-et-al:hcis2017,mcnaughton-et-al:jce2017}.
The framework supports numerous control technologies which teachers can use to control instances of the framework on student-centric multi-touch interfaces.
...
% TODO Trim down (subsection on potential issues could be summarised (they are just reasons for the limitations on acceptable gestures which we used to determine what we asked the participants.)
...
% TODO Trim a lot of this section.

% TODO Explain need for focus group.
...
\begin{itemize}
\item  \textit{\textbf{Cost:}} As detailed in Section~\ref{sec:orchestration}, \ac{UTToF} cameras are relatively low cost.
...
\label{subsubsec:focusGroupAnalysis}
...
When asked to modify their behaviour so that they did not use their fingers (due to the limitations of \ac{UTToF} cameras discussed in Section~\ref{sec:orchestration}) participants often performed Gesture 21 instead.
...
As discussed in Section~\ref{subsec:gatheringGestures}, criteria derived from the requirements outlined by Wachs et al.~\citeyearpar{Wachs2011} relating to the sensing technology are fulfilled by the proposed use of a \ac{UTToF} camera.
...
% TODO Brief intro to first stage of study

\subsection{Software implementation}
\label{subsec:studyPhase1Implementation}

% TODO Trim down and make appropriate to section.
