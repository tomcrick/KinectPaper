...
This could be rectified with more in-depth training in sub-sequent studies.
...
Improving the field of vision of the system could rectify this.
...
Again, this could be corrected with an improvement to the field of vision of the system.
...
The pilot study made it apparent that augmentations to the SynergyNet framework would be required to overcome the limitations of the Kinect.
One such augmentation identified to be beneficial would be the use of multiple Kinect devices to accurately monitor a larger area.
...
If the area accurately monitored by the Kinect was to be expanded to cover the entirety of a classroom, the teacher would then be capable of issuing commands through upper-body gestures anywhere in the environment.
...
With the use of multiple Kinect devices this issue would potentially be overcome.

To limit the errors caused by the Kinect losing track of the teacher's limbs and joints, it was decided that automatic calibration should be implemented~\cite{Bellmore2011}.
This would allow the teacher to issue commands through the upper-body gesture control sequence without performing the calibration pose beforehand which simplifies use of the system and allows for quicker recovery when the devices lose track of a user.
...
Therefore, to reduce the number of errors, it was decided that improvements to the gesture set should be made.
...
To assess the usage of the upper-body gesture control sequence derived in Section~\ref{subsec:userGeneratedGestures} it was a decided that a study should be carried out to compare their usage with more traditional methods of orchestrating technology in the classroom.
It was decided that the implementation created for the pilot study discussed in Section~\ref{sec:pilotStudy} could be built upon for the study. 
Section~\ref{subsec:pilotdiscussion} several issues were identified that would need to be resolved in the design of study.
...
The issues outlined in Section~\ref{subsec:pilotdiscussion} were noted in the previous phase of the study to make the use of the system counter-productive.
...
With clear definitions of the issues in Section~\ref{subsec:pilotdiscussion} and their potential solutions in Section~\ref{subsec:studyResolvingIssues}, improvements can be implemented in SynergyNet.
